...
MongoDB.prototype.log = function(info, cb) {
  ...
  process.nextTick(()=>{
    ...
    //updates according to social champ error needds and schema
    let errorInfo = info[Symbol.for('splat')][1];
    let userId = info[Symbol.for('splat')][0] || errorInfo.userId || errorInfo.accountId;
    entry.type = errorInfo.type || `Server-${info.level}`;
    entry.data = {
      userId: userId,
      postId: errorInfo.postId || (errorInfo.query && errorInfo.query.postId) || (errorInfo.params && errorInfo.params.postId) || undefined,
      url: errorInfo.url,
    ...
    entry.message = errorInfo.error && errorInfo.error.message || ''; 
    entry.stack = errorInfo.error && errorInfo.error.stack || 'no stack found';
    entry.created = entry.timestamp;
    entry.raw = JSON.stringify(errorInfo);
